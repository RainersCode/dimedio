...
export default function OrganizationSelector({ onError, onSuccess }: OrganizationSelectorProps) {
  const {
    activeMode,
    membershipStatus,
    allOrganizations,
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-6">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-slate-900 mb-2">Drug Inventory View</h3>
        <p className="text-sm text-slate-600">Switch between different drug inventories to manage medications for individual practice or organization teams.</p>
      </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              }`}>Individual Inventory</div>
              <div className={`text-sm ${
                activeMode === 'individual' ? 'text-blue-700' : 'text-slate-600'
              }`}>Your personal drug inventory</div>
            </div>
            {activeMode === 'individual' && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className={`text-sm ${
                  ...
                  {org.role === 'admin' ? 'Administrator' : 'Member'} • Organization inventory
                </div>
              </div>
      ...
      {switching && (
        <div className="mt-4 flex items-center justify-center py-2">
          ...
          <span className="text-sm text-slate-600">Switching inventory view...</span>
        </div>
      )}
    </div>
  );
